...
/// @brief Comprueba si una gramática es lineal por la derecha
/// @return true || false
bool Gramatica::esLinealPorLaDerecha() const {
  bool es_lineal_derecha = true;
  for (const auto &produccion : producciones_) {
    const std::vector<char> &simbolos = produccion.getProd();
    // Verificar si la producción es una sola terminal
    if (simbolos.size() == 1 && terminales_.isSymbol(simbolos[0])) {
      es_lineal_derecha = true;
      continue;
    }
    // Verificar producciones de longitud mayor o igual a 2
    if (simbolos.size() >= 2) {
      bool todos_terminales_antes = true;
      // Revisar todos los símbolos menos el último para asegurarse de que sean terminales
      for (size_t i = 0; i < simbolos.size() - 1; ++i) {
        if (!terminales_.isSymbol(simbolos[i])) {
          todos_terminales_antes = false;
          es_lineal_derecha = false;
          break;
        }
      }
      // Comprobar que el último símbolo sea un no terminal
      bool ultimo_es_no_terminal = no_terminales_.count(std::string(1, simbolos.back())) > 0;
      // La producción debe cumplir ambas condiciones para ser lineal por la derecha
      if (todos_terminales_antes && ultimo_es_no_terminal) {
        es_lineal_derecha = true;
        continue;
      }
      if (!es_lineal_derecha)
        return false;
    }
  }
  return true;  // Todas las producciones cumplen, es lineal por la derecha
}