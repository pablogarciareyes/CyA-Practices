...
/// @brief Constructor con el nombre de un fichero
/// @param nombre_fichero
Cadena::Cadena(const std::string& nombre_fichero) {
  readCadena(nombre_fichero);
}
...
/// @brief Leer la cadena desde un archivo
/// @param nombre_fichero
void Cadena::readCadena(const std::string& nombre_fichero) {
  std::ifstream file(nombre_fichero);
  if (file.is_open()) {
    std::string line;
    while (std::getline(file, line)) {
      for (char c : line) {
        cadena_vector_.push_back(c);
      }
    }
    file.close();
  } else {
